...
module.exports = (app) => {
  ...
  app.post('/api/v1/user/signup', validateUser.signup, validateUser.signin, usersController.create);
  ...
  app.post('/api/v1/group', authenticate, validateGroup.name, validateGroup.isEmptyContent, groupsController.create);

  // An API route that allow users add other users to groups:
  app.post('/api/v1/group/:groupid/user', authenticate, validateGroup.validGroup, validateGroup.isGroupMember, validateGroup.user, groupsController.addNewUser);


  // An API route that allows a logged in user post messages to created groups:
  app.post('/api/v1/group/:groupid/message', authenticate, validateGroup.isGroupMember, validateMessage.isNull, validateGroup.validGroup, messagesController.create);

  // An API route that allows a logged in user retrieve messages that have been
  // posted to groups he/she belongs to:
  app.get('/api/v1/group/:groupid/messages', authenticate, validateGroup.validGroup, validateGroup.isGroupMember, groupsController.list);

  // An API route that allows a logged in user list users in a group that he/she belongs to
  app.get('/api/v1/group/:groupid/users', authenticate, validateGroup.validGroup, validateGroup.isGroupMember, groupsController.listMembers);
